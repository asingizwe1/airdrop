...
DEFAULT_ZKSYNC_ADDRESS="0x36615Cf349d7F6344891B1e7CA7C72883F5dc049" #private key is above as local_key
...
echo "Creating zkSync local node..." #starts alocal node for zksync
npx zksync-cli dev start
echo "Deploying token contract..." #deployes contract
...
SIGNATURE=$(cast wallet sign --private-key ${DEFAULT_ANVIL_KEY} --no-hash ${MESSAGE_HASH}) #sign the message
CLEAN_SIGNATURE=$(echo "$SIGNATURE" | sed 's/^0x//')
echo -n "$CLEAN_SIGNATURE" >> signature.txt 

# Split signature 
SIGN_OUTPUT=$(forge script script/SplitSignature.s.sol:SplitSignature) #does wht the splitting of the v,r,s was 
